...
// =============================================================================
// Accumulator Module (Phase 1 Update, Rule-Aligned)
// - Multi-Latch checkpoint support (latch0/latch1)
// - L/R bank pipeline accumulation
// - Optional external control for bank/latch selection
// - Simplified control path (single always_comb + single always_ff)
// =============================================================================
...
module accumulator (
    ...
    // -------------------------------------------------------------------------
    logic        c_input_sign;
    logic [49:0] c_input_mag_q30;

    logic        c_lane_l_work_sign;
    logic [49:0] c_lane_l_work_mag;
    logic [9:0]  c_lane_l_work_exp;

    logic        c_lane_r_work_sign;
    logic [49:0] c_lane_r_work_mag;
    ...
    task automatic align_value (
        input  logic        in_sign,
        input  logic [49:0] in_mag,
        input  logic [9:0]  in_exp,
        input  logic [9:0]  target_exp,
        output logic        out_sign,
        output logic [49:0] out_mag
    );
        logic [9:0] diff;
        begin
            if (in_mag == 50'd0) begin
                out_sign = 1'b0;
                out_mag = 50'd0;
            end else begin
                ...
                if (diff >= 10'd50) begin
                    out_mag = 50'd0;
                    out_sign = 1'b0;
                end else begin
                    out_mag = in_mag >> diff[5:0];
                    if (out_mag == 50'd0) begin
                        out_sign = 1'b0;
                    end else begin
                        out_sign = in_sign;
                    end
    ...
    task automatic add_signed_mag (
        input  logic        a_sign,
        input  logic [49:0] a_mag,
        input  logic        b_sign,
        input  logic [49:0] b_mag,
        output logic        sum_sign,
        output logic [49:0] sum_mag
    );
        logic [50:0] add_ext;
        begin
            if (a_mag == 50'd0 && b_mag == 50'd0) begin
                sum_sign = 1'b0;
                sum_mag = 50'd0;
            end else if (a_mag == 50'd0) begin
                sum_sign = b_sign;
                sum_mag = b_mag;
            end else if (b_mag == 50'd0) begin
                sum_sign = a_sign;
                sum_mag = a_mag;
            end else if (a_sign == b_sign) begin
                add_ext = {1'b0, a_mag} + {1'b0, b_mag};
                if (add_ext[50]) begin
                    sum_mag = 50'h3FFFFFFFFFFFF;
                end else begin
                    sum_mag = add_ext[49:0];
                end
                if (sum_mag == 50'd0) begin
                    sum_sign = 1'b0;
                end else begin
                    sum_sign = a_sign;
                end
    ...
    task automatic normalize_q30 (
        ...
        begin
            if (in_mag == 50'd0) begin
                ...
                msb_idx = find_msb50(in_mag);

                if (msb_idx < 6'd30) begin
                    norm_mag = in_mag << (6'd30 - msb_idx);
                end else begin
                    norm_mag = in_mag >> (msb_idx - 6'd30);
                end

                exp_sum = {2'b00, in_exp} + {6'd0, msb_idx};

                if (exp_sum <= 12'd30) begin
                    out_sign = 1'b0;
                    out_mag = 50'd0;
                    out_exp = 10'd0;
                end else begin
                    exp_adj = exp_sum - 12'd30;
                    if (exp_adj > 12'd1023) begin
                        out_sign = in_sign;
                        out_mag = 50'h3FFFFFFFFFFFF;
                        out_exp = 10'h3FF;
                    end else begin
                        out_sign = in_sign;
                        out_mag = norm_mag;
    ...
    task automatic make_bf16_rne (
        ...
        begin
            if (in_mag == 50'd0) begin
                ...
                mant_tmp = {1'b0, in_mag[29:23]} + {7'd0, round_up};
    ...
    task automatic decode_bf16_to_q30 (
        ...
        begin
            ...
            if (exp_field == 8'd0) begin
                ...
                out_sign = in_bf16[15];
                out_mag = {19'd0, 1'b1, 7'h7F, 23'd0};
                out_exp = 10'd254;
            end else begin
                out_sign = in_bf16[15];
                out_mag = {19'd0, 1'b1, man_field, 23'd0};
    ...
    always_comb begin
        ...
        if (i_acc_clear) begin
            ...
            c_input_sign = i_sign;
            c_input_mag_q30 = {18'd0, i_ma_sum} << 8;

            if (!i_acc_en) begin
                c_lane_l_work_sign = 1'b0;
                c_lane_l_work_mag = 50'd0;
                c_lane_l_work_exp = 10'd0;

                c_lane_r_work_sign = 1'b0;
                c_lane_r_work_mag = 50'd0;
            ...
            if (c_update_prev_mag == 50'd0) begin
                c_update_common_exp = i_max_exp;
                c_update_prev_aligned_sign = 1'b0;
                c_update_prev_aligned_mag = 50'd0;
                c_update_input_aligned_sign = c_input_sign;
                c_update_input_aligned_mag = c_input_mag_q30;
            end else begin
            ...
            if ((c_lane_l_work_mag == 50'd0) && (c_lane_r_work_mag == 50'd0)) begin
                c_merge_sum_sign = 1'b0;
                c_merge_sum_mag = 50'd0;
                c_merge_common_exp = 10'd0;
            end else begin
