#!/usr/bin/env python3
"""Collect projection-output deltas from one full PIM-injected forward."""
...
def projection_module(layer: Any, projection: str) -> Any:
    module = layer
...
def capture_projection_outputs(
    model: Any,
    output_store: dict[tuple[int, str], Any],
    projections: list[str],
) -> list[Any]:
    handles = []

    def make_hook(layer_idx: int, projection: str):
        def hook(_module: Any, _inputs: tuple[Any, ...], output: Any) -> None:
            tensor = output[0] if isinstance(output, tuple) else output
            output_store[(layer_idx, projection)] = tensor.detach()
    ...
    for layer_idx, layer in enumerate(prop.decoder_layers(model)):
        for projection in projections:
            handles.append(
                projection_module(layer, projection).register_forward_hook(
                    make_hook(layer_idx, projection)
                )
            )
    return handles
...
def cuda_ready() -> None:
    ...
    for _attempt in range(5):
        try:
            ...
            last_error = exc
            time.sleep(2.0)
    raise RuntimeError("Full PIM projection-output run requires CUDA") from last_error


def emit_projection_rows(
...
def main() -> int:
    ...
    baseline_outputs: dict[tuple[int, str], Any] = {}
    pim_outputs: dict[tuple[int, str], Any] = {}
    ...
    try:
        ...
        with torch.inference_mode():
            ...
            handles = capture_projection_outputs(model, baseline_outputs, PROJECTIONS)
            try:
                _baseline = model(
                    input_ids=input_ids,
            ...
            handles = capture_projection_outputs(model, pim_outputs, PROJECTIONS)
            try:
                _pim = model(
                    input_ids=input_ids,
                    attention_mask=attention_mask,
        ...
        args.elementwise_output.parent.mkdir(parents=True, exist_ok=True)
        with args.elementwise_output.open("w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=ELEMENT_FIELDS, extrasaction="ignore")
            writer.writeheader()

            for layer_idx in range(num_layers):
                for projection in PROJECTIONS:
                    store_key = (layer_idx, projection)
                    if store_key not in baseline_outputs or store_key not in pim_outputs:
                        raise RuntimeError(f"Missing captured projection output for layer {layer_idx} {projection}")

                    gpu_bits, shape = selected_bits(baseline_outputs[store_key], args.all_tokens)
                    pim_bits, _shape = selected_bits(pim_outputs[store_key], args.all_tokens)
                    if gpu_bits.shape != pim_bits.shape:
                        raise RuntimeError(
                            f"Projection output shape mismatch for layer {layer_idx} {projection}: "
                            f"{gpu_bits.shape} != {pim_bits.shape}"
                        )
                    ...
                    key = {
                        ...
                        "tensor_name": "projection_output",
                    }
                    deltas = emit_projection_rows(writer, key, gpu_bits, pim_bits, shape)
    ...
    expected_summary_rows = num_layers * len(PROJECTIONS)
    metadata = {
        "model_id": args.model_id,
        ...
        "captured_tensor": "projection_output",
